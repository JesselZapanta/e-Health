...
$totalAppointments = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) total FROM appointments WHERE $date_condition AND status='Approved' AND doctor_id='$user_id'")
)['total'] ?? 0;

$checkinAppointments = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) total FROM appointments WHERE $date_condition AND status='Check-in' AND doctor_id='$user_id'")
)['total'] ?? 0;

$cancelledAppointments = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) total FROM appointments WHERE $date_condition AND status='Cancelled' AND doctor_id='$user_id'")
)['total'] ?? 0;

$completedAppointments = mysqli_fetch_assoc(
    mysqli_query($conn, "SELECT COUNT(*) total FROM appointments WHERE $date_condition AND status='Completed' AND doctor_id='$user_id'")
)['total'] ?? 0;
...
$res = mysqli_query(
    $conn,
    "SELECT status, COUNT(*) total FROM appointments WHERE $date_condition AND doctor_id='$user_id' GROUP BY status"
);
...
$res = mysqli_query(
    $conn,
    "SELECT type, COUNT(*) total FROM appointments WHERE $date_condition AND doctor_id='$user_id' GROUP BY type"
);
while ($r = mysqli_fetch_assoc($res)) {
